...
static constexpr bool kEnableRtt = false;
...
void printRttRaw(const BleCsRttSample& sample) {
  for (uint8_t i = 0U; i < sample.rawLen; ++i) {
    if (sample.rawBytes[i] < 16U) {
      Serial.print('0');
    }
    Serial.print(sample.rawBytes[i], HEX);
  }
}
...
void loop() {
  ...
  if ((now - gLastLogMs) >= 1000U) {
    ...
    if (gLastEstimateValid) {
      ...
      if (kEnableRtt && gLastEstimate.rttChannels > 0U &&
          isfinite(gLastEstimate.rttDistanceMeters)) {
        Serial.print(gLastEstimate.rttDistanceMeters, 4);
      } else {
        Serial.print(F("na"));
      }
      ...
      Serial.print(kEnableRtt ? gLastEstimate.rttChannels : 0U);
      Serial.print(F(" rtt_var="));
      if (kEnableRtt && gLastEstimate.rttChannels > 0U &&
          isfinite(gLastEstimate.rttVariance)) {
        Serial.print(gLastEstimate.rttVariance, 6);
      } else {
        Serial.print(F("na"));
      }
    } else {
