...
constexpr uint8_t kDefaultPayload[] = "xiao-nrf54l15-vpr-crc32c";
...
uint32_t hostCrc32c(const uint8_t* data, size_t len) {
...
bool runProbe(const uint8_t* data, size_t len, bool rebootService) {
  ...
  g_lastHostCrc = hostCrc32c(data, len);
  if (!g_service.crc32c(data, len, &g_lastVprCrc, &g_lastProcessedLen)) {
    return false;
  }
...
void setup() {
  ...
  delay(1200);
  Serial.println();
  Serial.println("VPR CRC32C offload probe");
  printHelp();
